...
public class QLearningDataFetcher extends BaseDataFetcher {
    ...
    Learner.InputType type;
    public QLearningDataFetcher(int n, Learner.InputType type){
        if (type.equals(Learner.InputType.Convolution)) Inputs = new ArrayList<>();
        else if (type.equals(Learner.InputType.Dense))InputsDense  = new ArrayList<>();
    ...
    public void AddNewValueDense(double[] input, double[] output){
        ArrayList<double[]> inp = new ArrayList<>(InputsDense.size()+1);
    ...
    public DataSet getDataSet(){
        if (type.equals(Learner.InputType.Dense)){
        ...
        else if (type.equals(Learner.InputType.Convolution)){
    ...
    public void fetch(int numExamples) {
        ...
        if (actualExamples < numExamples) {
            ...
            if (type.equals(Learner.InputType.Convolution)) labelData = Arrays.copyOfRange(labelData, 0, actualExamples);
            else if (type.equals(Learner.InputType.Dense)) inputDense = Arrays.copyOfRange(inputDense, 0, actualExamples);
        }
        INDArray features = (type.equals(Learner.InputType.Convolution))?Nd4j.create(featureData):Nd4j.create(inputDense);
